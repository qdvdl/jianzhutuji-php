...
class PHPExcel
{
    ...
    */
	...
	//自定义读取excel数据
	public function excel_reader($filename){
		$exts=$this->getExtensionOnly($filename);
		if($exts=="xls"){
			$Excel_Reader5=new PHPExcel_Reader_Excel5();
			return $Excel_Reader5;
		}else if($exts=="xlsx"){
			$Excel_Reader2007=new PHPExcel_Reader_Excel2007();
			return $Excel_Reader2007;
		}
	}
	//自定义写入excel导出
	public function excel_writer($objPHPExcel){
		$objWriter = new PHPExcel_Writer_Excel5($objPHPExcel);
		return $objWriter;
	}
	
	//导出文件excel
	public function export_file_excel($filename,$field,$datas){
		echo time();
		//实例
		$phpexcel=$this;
		//设置文件信息
		...
		->setCreator("admin")          //作者
		->setLastModifiedBy("admin")   //最后一次保存作者
		->setTitle("test")            //标题
		->setSubject("test1")         //主题
		->setDescription("test")	  //备注
		->setKeywords("excel")		  //标记
		->setCategory("file");       //类别
		//设置表头
		$excel_key=['A','B','C','D','E','F','G','H','I','J','K','L','M','N'];
		//默认打开第一个工作表
		$objExcel=$phpexcel->setActiveSheetIndex(0);
		//设置表头
		$keys=0;
		$e_key;
		foreach($field as $key=>$item){
			$objExcel->setCellValue($excel_key[$keys].'1', $item);	
			$e_key=$excel_key[$keys];
			$keys++;
		}
		//表头
		$set_key="A1:".$e_key."1";
		...
		//设置第一行文本颜色(单个)
		$objExcel->getStyle($set_key)->getFont()->setBold(true)->getColor()->setARGB('00000000');
		//设置表头填充颜色
		$objExcel->getStyle($set_key)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
 		$objExcel->getStyle($set_key)->getFill()->getStartColor()->setARGB("00FFA500");
		
		//设置边框
		//PHPExcel_Style_Border::BORDER_THICK,//边框是粗的  
        //PHPExcel_Style_Border::BORDER_THIN,//细边框  
		$styleArray = array(
			//设置边框
			'borders' => array(
				'allborders' => array(
		...
		foreach($datas as $key=>$data){
			//添加样式
			$objActSheet->getStyle('A1:'.$e_key.$i)->applyFromArray($styleArray);
			$k=0;
			foreach($data as $key=>$item){
				$objActSheet->setCellValue($excel_key[$k].$i, $data[$key]);
				//$objPHPExcel->getActiveSheet(0)->setCellValueExplicit($excel_key[$k].$i,$data[$key],\PHPExcel_Cell_DataType::TYPE_STRING);//转换科学计数法
				//$objActSheet->getStyle($excel_key[$k].$i)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER);
				$k++;
			}
		 	$i++;
		}
		
//		$objPHPExcel->getActiveSheet()->getColumnDimension($item)->setAutoSize(true);//自适应
		$allRow=$objExcel->getHighestRow();  
		//固定默认行高
		$objActSheet->getDefaultRowDimension()->setRowHeight(15);
		$objActSheet->freezePane('A2');
		//重命名设置标题
		$objActSheet->setTitle($filename. date('Y-m-d'));
		//设置第一行行高
		//$this->phpexcel->getActiveSheet()->getRowDimension('1')->setRowHeight(30);
		//设置列宽
		$objActSheet->getColumnDimension('A')->setWidth(18.5);
		$objActSheet->getColumnDimension('B')->setWidth(10.5);
		...
		$file_path="./upload/excel/export_log/".$filename;
		//接受上传数据
		ob_end_clean();//清除缓冲区,避免乱码 
		$objWriter=$phpexcel->excel_writer($phpexcel);//添加数据到表中，写入数据
		$objWriter->save($file_path);//存储文件
		return $file_path;
		exit;
		ob_end_clean();//清除缓冲区,避免乱码 
